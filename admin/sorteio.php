...
if (($_GET['ajax'] ?? '') === 'reveal') {
  ...
  if ($id <= 0 || !in_array($field, ['cpf', 'voto'], true)) {
    echo json_encode(['ok' => false, 'message' => 'Parâmetros inválidos.']);
    exit;
  }

  $stmt = $pdo->prepare('SELECT v.eleitor_cpf, c.nome candidato_nome
    FROM votos v
  ...
  if (!$row) {
    echo json_encode(['ok' => false, 'message' => 'Registro não encontrado.']);
    exit;
  }

  if ($field === 'cpf') {
    echo json_encode(['ok' => true, 'value' => formatCpf($row['eleitor_cpf'])]);
    exit;
  }

  echo json_encode(['ok' => true, 'value' => $row['candidato_nome']]);
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'sortear') {
  $stmt = $pdo->query('SELECT v.id voto_id, v.codigo_sorteio, v.eleitor_nome, v.eleitor_empresa, v.eleitor_turno, v.eleitor_setor
    FROM votos v
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2 no-print">
  ...
  <div class="d-flex gap-2">
    <button class="btn btn-outline-secondary" type="button" id="fullscreenBtn">Tela cheia</button>
    <a class="btn btn-outline-secondary" href="<?= e(url('admin/dashboard.php')) ?>">Voltar</a>
  </div>
</div>

<div class="card p-3 p-md-4 sorteio-stage" id="sorteioStage">
  ...
  <form method="post" class="text-center mb-4 no-print" id="drawForm">
  ...
  <?php if ($winner): ?>
  <div id="winnerCard" class="winner-card text-center" data-voto-id="<?= (int)$winner['voto_id'] ?>">
    ...
    <div class="d-flex justify-content-center flex-wrap gap-2 mt-2 no-print">
      <button type="button" class="btn btn-outline-secondary btn-sm" id="toggleCpfBtn" data-hidden="1">Mostrar CPF</button>
      <button type="button" class="btn btn-outline-secondary btn-sm" id="toggleVoteBtn" data-hidden="1">Mostrar voto</button>
    </div>
...
(function () {
  ...
  const stage = document.getElementById('sorteioStage');
  ...
  const voteId = winnerCard ? winnerCard.dataset.votoId : '';

  async function fetchSensitive(field) {
    const url = new URL(window.location.href);
    url.searchParams.set('ajax', 'reveal');
    url.searchParams.set('field', field);
    url.searchParams.set('id', voteId || '0');
  ...
  if (cpfBtn && cpfField) {
    let cpfValue = '';
    cpfBtn.addEventListener('click', async function () {
      const hidden = cpfBtn.dataset.hidden === '1';
      if (hidden) {
        if (!cpfValue) cpfValue = await fetchSensitive('cpf');
        cpfField.textContent = 'CPF: ' + cpfValue;
  ...
  if (voteBtn && voteField) {
    ...
    voteBtn.addEventListener('click', async function () {
      ...
      if (hidden) {
        if (!voteValue) voteValue = await fetchSensitive('voto');
        voteField.textContent = 'Voto em: ' + voteValue;
